...
output:
  beamer_presentation:
    ...
    slide_level: 2
    includes:
      in_header: preamble_reallywidehat.tex
...
+ Note that, even though we have estimated a linear probability model, the null hypothesis we are testing is that
all slope coefficients are zero, and so there is no heteroskedasticity under H0. This means that the usual F-statistic is asymptotically valid.
...
+ For the LPM, standard errors should be made robust to heteroskedasticity
...
+ For PROBIT, there is no advantage in using robust standard errors. [(reference)](https://blog.stata.com/2016/08/30/two-faces-of-misspecification-in-maximum-likelihood-heteroskedasticity-and-robust-standard-errors/#disqus_thread)
  + They do not solve the problems associated with heteroskedasticity for a nonlinear model estimated using maximum likelihood;  do not help us obtain correct inference.
...
+ The test statistic follows the chi-square distribution (denoted by $\chi^2$), with degrees of freedom equal to the
number of regressors.

+ The likelihood ratio test for joint significance is $\chi^2 = 10.18$.

+ In a $\chi^2$ distribution this gives $p-value = .18$, which is very similar to that obtained for the LPM in part (ii).
...
$\Rightarrow$ Predicted probabilities of  being unemployed in 1978 \footnotesize [(>>stata)](#LMPsimplereg_predict) \normalsize
  ...
  + when $train = 1$ is: $\reallywidehat{unem78}(train = 0) = .354 - .111 = \mathbf{.243}$
...
## (vii) Which approach would you use to measure the effect and statistical significance of the job training program?
...
\small
```{stata, eval=FALSE, fig.show='hold', fig.align='center', fig.pos = 'H'}
* probit yvar xvar wvar1 wvar2 wvark, robust
* predict newvar, p
...
\small
We observe a still very high correlation of $.9932$. This is due to the fact that the explanatory variables other than `train` are insignificant (with the exception of `black`). Therefore, the predicted probabilities are still primarily determined by `train`, and hence they are highly correlated.

## [SN] STATA command for Average Partial Effects
\small

```{stata, eval=FALSE, fig.show='hold', fig.align='center', fig.pos = 'H'}
* probit yvar ib0.binary_varname c.continuous_varname, robust
// use 'c.' to explicitly indicate continuous variables
// use 'ib0.' to indicate binary variables, with base value 0
```

```{stata, eval=FALSE, fig.show='hold', fig.align='center', fig.pos = 'H'}
* probit yvar ib0.binary_var c.continuous_var, robust
...
+ **Objective:** test whether *participation in the job training program*
had an effect on *unemployment probabilities* and *earnings* in 1978.
...
We observe that the APE for regattend is about $.0950 (t = 6.44)$. So, the APE estimate and its
